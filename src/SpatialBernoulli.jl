...
"""
struct SB{TR <: Real, TS <: Real, TO <: Real, AV <: AbstractVector, AM <: AbstractMatrix, AAM <: AbstractMatrix} <: DiscreteMultivariateDistribution
	range::TR
	sill::TS
	order::TO
	λ::AV
...
"""
function pdf(d::SB, y::AbstractVector{<:Real}; m = length(d) * 500, return_error = false, a = zeros(length(d)), b = zeros(length(d)), finite_bounds = zeros(length(d)), zerosvec = fill(0, length(d)))
...
logpdf(d::SB, y::AbstractVector{<:Real}; m = length(d) * 500, return_error = false, a = zeros(length(d)), b = zeros(length(d)), finite_bounds = zeros(length(d)), zerosvec = fill(0, length(d))) =
	log(pdf(d, y; m = m, return_error = return_error, a = a, b = b, finite_bounds = finite_bounds, zerosvec = zerosvec))

function logpdf(d::SB, y::AbstractVector{<:Real}, wp::AbstractMatrix{<:Real}; m = 2 * 100, return_error = false)
	...
	diagonal_sum = sum(wp[i, i] * logpdf(SB(d.range, d.sill, d.order, d.λ[[i, i]], d.h[[i, i], [i, i]]), y[[i, i]]; m = m) for i in 1:length(d))
...
loglikelihood(d::SB, Y::AbstractArray{<:Real}; m = length(d) * 500) = sum(logpdf(d, c; m = m) for c in eachcol(Y))


loglikelihood(d::SB, Y::AbstractArray{<:Real}, w::AbstractVector{<:Real}; m = length(d) * 500) = sum(w[i] * logpdf(d, c; m = m) for (i, c) in enumerate(eachcol(Y)))

function loglikelihood(d::SB, Y::AbstractArray{<:Real}, wp::AbstractMatrix{<:Real}; m = 2 * 100)
	ds, n = size(Y)
	# Pairwise indices where weights[i, j] > 0
	...
	for (i, j) in pairwise_indices2
		...
		if i == j
			...
			Iij1[i, j] = max(eps, ifelse(Iij1[j, i] != 1.0, Iij1[j, i], pdf(
				SB(d.range, d.sill, d.order, B_ij, h_ij), [1, 1]; m = m)
			))
...
function loglikelihood(d::SB, Y::AbstractArray{<:Real}, wp::AbstractMatrix{<:Real}, w::AbstractVector{<:Real}; m = 2 * 100)
	ds, n = size(Y)
	# Pairwise indices where weights[i, j] > 0
	...
	Iij3 = ones(eltype(d.ΣU), ds, ds)
	for (i, j) in pairwise_indices2
		Iij3[i, j] = ifelse(Iij2[j, i] != 1.0, Iij2[j, i], pdf(SB(d.range, d.sill, d.order, d.λ[[i, j]], d.h[[i, j], [i, j]]), [0, 1]; m = m))
...
function loglikelihood_vfast(d::SB, Y::AbstractArray{<:Real},
